...
<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            ...
            @if(! $showDocumentLogin)
                <div class="flex items-center gap-3">
                    <span class="text-xs font-semibold uppercase tracking-wider text-emerald-600">{{ $documents->count() }} records</span>
                    <div class="rounded-xl border border-emerald-100 bg-emerald-50 px-3 py-2 text-left shadow-sm">
                        <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-emerald-600">Signed in to Document Tracking</p>
                        <p class="mt-1 text-sm font-semibold text-slate-800">{{ $documentTrackingUser?->username ?? auth()->user()->username }}</p>
                        <p class="text-[11px] text-slate-500">{{ $documentTrackingUser?->role ?? auth()->user()->role }}</p>
                    </div>
                    <form method="POST" action="{{ route('documents.logout') }}">
                        @csrf
                        <button type="submit" class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-50">Log out</button>
                    </form>
                </div>
            @endif
        </div>
    ...
    <div class="py-8">
        <div class="mx-auto w-full max-w-[98%] space-y-6 sm:px-6 lg:px-8">
            @if($showDocumentLogin)
                <div class="mx-auto max-w-lg rounded-2xl border border-emerald-100 bg-white p-8 shadow-sm">
                    <div class="mb-6 text-center">
                        ...
                        <h3 class="text-2xl font-bold text-slate-900">Document Tracking Module</h3>
                        <p class="mt-2 text-sm text-slate-500">Sign in to access the records assigned to your RBAC segment.</p>
                    </div>

                    @if($errors->any())
                        <div class="mb-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{{ $errors->first() }}</div>
                    @endif
